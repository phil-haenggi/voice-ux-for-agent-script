...
v0.3.4 — `developing-agentforce` dependency is now enforced at activation as **Phase 0 (dependency gate)**. Skill checks the filesystem itself before voicing the opening frame; refuses to proceed and surfaces the install command if missing. No more sliding through the flow without the companion installed.

**Earlier:** v0.3.3 / v0.3.2 tightened README framing. v0.3.1 framed the skill for both PM and developer audiences. v0.3.0 made `developing-agentforce` (afv-library) mandatory at intake alongside source script and sample conversation; added Phase 7 (deployable `.agent` generation) and Phase 8 (publish with self-healing against known-error catalog); added 40-file pattern catalog operationalizing the abstract principles, plus pattern linter and test fixtures. v0.2.2 made sample conversation mandatory (no synthetic fallback). v0.2.1 refined criteria for openings, two-voice contrast, latency, and grounding. v0.2.0 added optimize mode, mode auto-detection, mis-application checks, and mode-dependent conflict policy. Renamed from `migrating-text-to-voice` (v0.1.0).
